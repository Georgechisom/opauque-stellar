...
function VersionBadge({ info }: { info: ContractVersionInfo }) {
  ...
  if (info.status === "major-mismatch") {
    return (
      <span className="inline-flex items-center gap-1 rounded-full border border-neutral-500/30 bg-neutral-500/10 px-2 py-0.5 text-[11px] font-mono font-medium text-neutral-400">
        <span className="h-1.5 w-1.5 rounded-full bg-neutral-400" aria-hidden />
        {formatVersion(info.version)} ⚠
      </span>
    );
...
function MismatchWarning({ versions }: { versions: ContractVersionInfo[] }) {
  ...
  return (
    <div className="rounded-xl border border-neutral-500/30 bg-neutral-500/5 px-4 py-3 text-sm">
      <p className="font-semibold text-neutral-400 mb-1">Major version mismatch detected</p>
      ...
      <ul className="mt-2 space-y-0.5">
        {mismatches.map((m) => (
          <li key={m.contractId} className="text-xs font-mono text-neutral-400">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-mist mt-0.5">
            Expected major version: <span className="font-mono text-white">v{EXPECTED_MAJOR_VERSION}</span>
          </p>
        </div>
        <button
          type="button"
